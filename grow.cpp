...
//        double y0, y1;
        double Cgr;
        double y0=(Wal[0]->Get(XYAddress.x,XYAddress.y).CarbonAgar+Wal[0]->Get(XYAddress.x-1,XYAddress.y).CarbonAgar+Wal[0]->Get(XYAddress.x,XYAddress.y-1).CarbonAgar+Wal[0]->Get(XYAddress.x-1,XYAddress.y-1).CarbonAgar)/4.;
        double y1=(Env.Get(XYAddress.x,XYAddress.y,XYAddress.z).Carbon+Env.Get(XYAddress.x-1,XYAddress.y,XYAddress.z).Carbon+Env.Get(XYAddress.x,XYAddress.y-1,XYAddress.z).Carbon+Env.Get(XYAddress.x-1,XYAddress.y-1,XYAddress.z).Carbon)/4.;
    ...
    if (XYAddress.z>0)
    {
        double lambda=cm.z/BoxLength-floor(cm.z/BoxLength);
        double Cgr;
        double y0=(Env.Get(XYAddress.x,XYAddress.y,XYAddress.z-1).Carbon+Env.Get(XYAddress.x-1,XYAddress.y,XYAddress.z-1).Carbon+Env.Get(XYAddress.x,XYAddress.y-1,XYAddress.z-1).Carbon+Env.Get(XYAddress.x-1,XYAddress.y-1,XYAddress.z-1).Carbon)/4.;
        double y1=(Env.Get(XYAddress.x,XYAddress.y,XYAddress.z).Carbon+Env.Get(XYAddress.x-1,XYAddress.y,XYAddress.z).Carbon+Env.Get(XYAddress.x,XYAddress.y-1,XYAddress.z).Carbon+Env.Get(XYAddress.x-1,XYAddress.y-1,XYAddress.z).Carbon)/4.;
        Cgr = y1*lambda+y0*(1.-lambda);
        Cgr = Cgr/(Cgr+KC);
        Growth_rate = maxGrowthRate*max(0.0,Cgr-Maintenance_rate/C_rate);
	...
	Growth_rate = Growth_rate * SigmaMoL; // log(3)/log(2)
...
void divide(Cell& mother, Cell& daughter, double t){
    ...
    // The toggle's states are same between d
//    daughter.G = mother.G;
//    daughter.R = mother.R;
    double proteinPatFact = dl / (L + 2* mother.Radius);
    int motherR = int( mother.R / 2. +  proteinPatFact*mother.R) ;
    int motherG = int( mother.G / 2. +  proteinPatFact*mother.G) ;
    daughter.R = mother.R - motherR;
    mother.R = motherR;
    daughter.G = mother.G - motherG;
    mother.G = motherG;
    daughter.nextReaction = mother.nextReaction;
}
