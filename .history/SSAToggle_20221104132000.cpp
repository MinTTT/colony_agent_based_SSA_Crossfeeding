 ...
 * @param g GFP number
 * @param r RFP number
 * @param[out] x pin of the matrix for storing the 
 */
template<size_t rows, size_t cols>
...
int runSim(const double& timeNow, Cell* pCell){
    ...
    initPars((int)(pCell->G), (int)(pCell->R), x, u1);

    while (timeCell <= timeNow){
        updateP(x, pCell->GrowthRate / SigmaMoL, p);
        sumPropensity = sum(p);
        generateTau(uniDist, gen, sumPropensity, &tau); // Generate tau
        timeCell +=tau;
    ...
    pCell->G = static_cast<double>(x[0]);
    pCell->R = static_cast<double>(x[1]);
    return 0;
}
