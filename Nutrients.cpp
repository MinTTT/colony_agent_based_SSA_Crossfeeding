...
#pragma omp parallel for default(shared) private(zi,yi,xi,positionColony,positionAgar,positionWall,previousCarbon) reduction(+:conv)
            ...
            if (zi*dir>0)
            {
                for (yi=minY; yi<=maxY; yi++)
                {
                    for (xi=minX; xi<=maxX; xi++)
                    {
                        ...
                        previousCarbon = PreviousColony->Get(positionColony).Carbon;
                ...
                for (int level=0; level<maxLevels; level++)
                {
                    for (yi=0; yi<BoxY; yi++)
                    {
                        for (xi=0; xi<BoxX; xi++)
                        {
                            positionWall.x=xi;
                            positionWall.y=yi;
                            previousCarbon = PreviousWall[0]->Get(positionWall).CarbonAgar;
                            ...
                            // check convergence criteria for this grid point
                            if (fabs(CurrentWall[0]->Get(positionWall).CarbonAgar - previousCarbon) > ConvCrit * fabs(CurrentWall[0]->Get(positionWall).CarbonAgar)){
                                conv +=1;
                            }
//                            conv += int(fabs(CurrentWall[0]->Get(positionWall).CarbonAgar - previousCarbon) > ConvCrit*fabs(CurrentWall[0]->Get(positionWall).CarbonAgar));
                        }
                    }
                }
            }
                ...
                for (int level=0; level<maxLevels; level++)
                {
                    for (yi=0; yi<BoxY; yi++)
                    {
                        for (xi=0; xi<BoxX; xi++)
                        {
                            ...
                            previousCarbon = PreviousAgar[0]->Get(positionAgar).CarbonAgar;
                            UpdateAgar(CurrentAgar, PreviousAgar, CurrentWall, positionAgar, level);
                            // check convergence criteria for this grid point
                            if (fabs(CurrentAgar[0]->Get(positionAgar).CarbonAgar - previousCarbon) > ConvCrit * fabs(CurrentAgar[0]->Get(positionAgar).CarbonAgar)){
                                conv +=1;
                            }
//                            conv += int(fabs(CurrentAgar[0]->Get(positionAgar).CarbonAgar - previousCarbon) > ConvCrit*fabs(CurrentAgar[0]->Get(positionAgar).CarbonAgar) );
                        }
                    }
                }
            }
...
// Calculate the steady states of Nutrients
void UpdateEnvironment(Array3D<LocalEnv>* Env, Array3D<LocalEnv>* prevEnv, Array2D<LocalAga>* prevWal, Array3D<double>& DensityShiftP, Array3D<double>& Density1ShiftP,Array3D<double>& Density2ShiftP,Array2D<double>& WallDensityShiftP,IntCoord position, Array3D<double>& insideColonyDen)
{
    ...
    if (insideColonyDen.Get(position)>0.3 && position.z>0)
    {
        int ix = position.x;
        int iy = position.y;
        int iz = position.z;
...
void UpdateAgar(Array3D<LocalAga>** Aga, Array3D<LocalAga>** prevAga, Array2D<LocalAga>** Wal, IntCoord position, int level)
{
    ...
    {
        Cyy = (prevAga[level]->Get(position.x,position.y-1,position.z).CarbonAgar+prevAga[level]->Get(position.x,position.y+1,position.z).CarbonAgar-2.0*prevAga[level]->Get(position.x,position.y,position.z).CarbonAgar)/(BoxLength*BoxLength*pow(4.0,level));
    }
