...
//	// leakage, k, n, p
//	return leakage + (1.0 - leakage) / (1.0  + pow(p/k, n ));
//}
//
//double alphaG(const double &gr){
//	return 10. * gr*(16.609+ 627.747/(1.0+ pow(gr/0.865, 4.635)));
//}
//
//double alphaR(const double &gr){
//	return 10. * gr*( 26.836 + 320.215/ (1.0+ pow(gr/0.661, 4.09)));
//}

void updateToggle(const double &dt, Cell* pNewCell);

// integrates to find the new positions based on the forces due to neighbors
void integrate(const double &dt, const int &cellID, const Cell* old_cells, Cell* new_cells, const int* neighbours, DoubleArray2D& Height, CoordArray2D& Normal, UniformGrid& Grid, const IntCoord& XYAddress, DoubleArray2D& Wall)
{
...
//
//    updateToggle(dt, &new_cells[cellID]);  //func for ODE
}

// Update the toggle switch states.
void updateToggle(const double &dt, Cell* pNewCell){
	double gr = pNewCell->GrowthRate / SigmaMoL; // elongation rate to mass growth rate

    pNewCell->R += dt * (alphaR(gr) * hillFunc(tau_R, kG, nG, pNewCell->G) - (gr + deltaP) * pNewCell->R);
    pNewCell->G += dt * (alphaG(gr) * hillFunc(tau_G, kR, nR, pNewCell->R) - (gr + deltaP) * pNewCell->G);

}


// Integrate positions forward in time by dt, velocities by dt/2
void UpdatePositions(const double &dt, const DoubleCoord& F, DoubleCoord& T, const Cell& old_cell, Cell& new_cell)
{
...
void UpdateVelocities(const double &dt, const DoubleCoord& F, DoubleCoord& T, const Cell& old_cell, Cell& new_cell)
{
	// sets all properties to be the same
	new_cell = old_cell;
