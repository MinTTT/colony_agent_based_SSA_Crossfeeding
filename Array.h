...
public:
	...
	void Set(int xi, int yi, T value)
	{
        ...
        if (m_Size.x*m_Size.y<=index){
            printf("2D array Size: %d X %d, invalid index: %d.", m_Size.x, m_Size.y, index);
            MyAssert(false,"Invalid index into 2D array ");
        }
	...
	{
		MyAssert(m_Size.x*m_Size.y==Length,"Incorrect array 2D size.");
...
public:
	...
	void Set(int xi, int yi, int zi, T value)
	{
        ...
        if (m_Size.x*m_Size.y*m_Size.z<=index){
            printf("3D array Size: %d X %d X %d, invalid index: %d.", m_Size.x, m_Size.y, m_Size.x, index);
            MyAssert(false,"Invalid index into 3D array ");
        }
//		MyAssert(m_Size.x*m_Size.y*m_Size.z>index,"Invalid index into 3D array");
		Data[index] = value;
	}

	void SetData(T* DataArray, int Length)
	{
		MyAssert(m_Size.x * m_Size.y * m_Size.z == Length, "Incorrect 3D array size");
		memcpy( Data, DataArray, Length*sizeof(T) );
	}
